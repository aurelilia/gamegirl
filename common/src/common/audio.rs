...
use super::options::SystemConfig;
...
pub fn make_sampler(
    ...
    match typ {
        AudioSampler::Nearest => Box::new(
            FastFixedOut::new(ratio, 1.0, PolynomialDegree::Nearest, output_buffer_len, 2).unwrap(),
        ),
        AudioSampler::Linear => Box::new(
            FastFixedOut::new(ratio, 1.0, PolynomialDegree::Linear, output_buffer_len, 2).unwrap(),
        ),
        AudioSampler::Cubic => Box::new(
            FastFixedOut::new(ratio, 1.0, PolynomialDegree::Cubic, output_buffer_len, 2).unwrap(),
        ),
...
impl AudioBuffer {
    ...
    pub(super) fn reinit_sampler(&mut self) {
