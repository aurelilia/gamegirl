...
impl<S: ArmSystem> SysWrapper<S> {
    ...
    // THUMB.1/2
    pub(crate) fn thumb_arithmetic<const KIND: &'static str>(&mut self, inst: ThumbInst) {
        let d = inst.low(0);
        let s = inst.low(3);
    ...
    // THUMB.3
    pub(crate) fn thumb_3<const KIND: &'static str>(&mut self, inst: ThumbInst) {
        let d = inst.low(8);
        let n = inst.0 & 0xFF;
        let rd = self.low(d);
        match KIND {
    ...
    // THUMB.4
    pub(crate) fn thumb_alu(&mut self, inst: ThumbInst) {
        let d = inst.low(0);
        let s = inst.low(3);
    ...
    // THUMB.5
    pub(crate) fn thumb_hi_add(&mut self, inst: ThumbInst) {
    ...
    pub(crate) fn thumb_hi_cmp(&mut self, inst: ThumbInst) {
        let s = inst.0.bits(3, 4);
        let d = inst.low(0) | (inst.0.bit(7) << 3);
    ...
    pub(crate) fn thumb_hi_mov(&mut self, inst: ThumbInst) {
        let s = inst.0.bits(3, 4);
        let d = inst.low(0) | (inst.0.bit(7) << 3);
        self.set_reg(d.u32(), self.reg(s.u32()));
    }

    pub(crate) fn thumb_hi_bx(&mut self, inst: ThumbInst) {
        let s = inst.0.bits(3, 4);
        if inst.0.is_bit(7) {
            // BLX
    ...
    // THUMB.6
    pub(crate) fn thumb_ldr6(&mut self, inst: ThumbInst) {
        let d = inst.low(8);
        let n = inst.0 & 0xFF;
    ...
    // THUMB.7/8
    pub(crate) fn thumb_ldrstr78<const O: u16>(&mut self, inst: ThumbInst) {
        let d = inst.low(0);
    ...
    // THUMB.9
    pub(crate) fn thumb_ldrstr9<const O: u16>(&mut self, inst: ThumbInst) {
        let d = inst.low(0);
    ...
    // THUMB.10
    pub(crate) fn thumb_ldrstr10<const STR: bool>(&mut self, inst: ThumbInst) {
        let d = inst.low(0);
        let n = inst.0.bits(6, 5);
        let rb = self.cpu().low(inst.low(3));
    ...
    // THUMB.11
    pub(crate) fn thumb_str_sp(&mut self, inst: ThumbInst) {
        let n = inst.0 & 0xFF;
        let d = inst.low(8);
        let rd = self.low(d);
    ...
    pub(crate) fn thumb_ldr_sp(&mut self, inst: ThumbInst) {
        let n = inst.0 & 0xFF;
        let d = inst.low(8);
        // LDR has +1I
        self.idle_nonseq();
    ...
    // THUMB.12
    pub(crate) fn thumb_rel_addr<const SP: bool>(&mut self, inst: ThumbInst) {
        let n = inst.0 & 0xFF;
        let d = inst.low(8);
        if SP {
    ...
    // THUMB.13
    pub(crate) fn thumb_sp_offs(&mut self, inst: ThumbInst) {
        let n = (inst.0 & 0x7F) << 2;
        let sp = self.cpur().sp();
        if inst.0.is_bit(7) {
    ...
    // THUMB.14
    pub(crate) fn thumb_push<const SP: bool>(&mut self, inst: ThumbInst) {
    ...
    pub(crate) fn thumb_pop<const PC: bool>(&mut self, inst: ThumbInst) {
        let mut sp = self.cpu().sp();
        let mut kind = NonSeq;
        // POP
        for reg in 0..8 {
    ...
    // THUMB.15
    pub(crate) fn thumb_stmia(&mut self, inst: ThumbInst) {
        let b = inst.low(8);
        let mut kind = NonSeq;
    ...
    pub(crate) fn thumb_ldmia(&mut self, inst: ThumbInst) {
        let b = inst.low(8);
        let mut kind = NonSeq;
        for reg in 0..8 {
            if inst.0.is_bit(reg) {
    ...
    // THUMB.16
    pub(crate) fn thumb_bcond<const COND: u16>(&mut self, inst: ThumbInst) {
    ...
    // THUMB.17
    pub(crate) fn thumb_swi(&mut self, _inst: ThumbInst) {
        self.swi();
    }

    // THUMB.18
    pub(crate) fn thumb_br(&mut self, inst: ThumbInst) {
        let nn = (inst.0.i10() as i32) * 2; // Step 2
        self.set_pc(self.cpur().pc().wrapping_add_signed(nn));
    }

    // THUMB.19
    pub(crate) fn thumb_set_lr(&mut self, inst: ThumbInst) {
        let lr = self
            .cpur()
            .pc()
    ...
    pub(crate) fn thumb_bl<const THUMB: bool>(&mut self, inst: ThumbInst) {
        let pc = self.cpu().pc();
...
impl ThumbInst {
    pub(crate) fn low(self, idx: u16) -> u16 {
        self.0.bits(idx, 3)
    }
}
